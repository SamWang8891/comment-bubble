...
/**
 * Render the comments list to the page
 * @param {string} hostname - Server hostname
 * @param {Array<string>} comments - Array of comment strings
 */
function renderRecord(hostname, comments) {
    let displayHTML = '';

    // Check if we received the new format (array) or old format (object)
    const commentArray = Array.isArray(comments) ? comments : Object.values(comments);
    ...
    if (commentArray.length > 0) {
        ...
        // Render each comment as a DOM structure
        commentArray.forEach((comment, index) => {
            const commentNumber = commentArray.length - index;
            displayHTML += `
                <div class="list-grid">
                    ...
                    <div>
                        <button
                            type="button"
                            class="short-delete-button js-delete-comment-button"
        ...
        displayHTML = `<div class="text-color">No comments found.</div>`;
        const searchDeleteDiv = document.querySelector('.js-search-delete-field-div');
        if (searchDeleteDiv) {
            searchDeleteDiv.innerHTML = `
    ...
    if (recordsContainer) {
        ...
        recordsContainer.querySelectorAll('.js-delete-comment-button').forEach((button) => {
            button.addEventListener('click', () => {
                const commentText = button.dataset.commentText;
                if (confirm(`Are you sure you want to delete this comment?\n\n"${commentText}"`)) {
                    // For now, we'll need to implement comment deletion in the backend
                    // This is a placeholder - you'll need to add a delete endpoint
                    alert('Comment deletion not yet implemented in backend');
                }
            });
        });
    }
